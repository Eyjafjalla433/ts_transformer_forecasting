...
CHECKPOINT_PATH = Path("checkpoints/NASDAQ_RNN.pt")
OUTPUT_PREDICTIONS = Path("outputs/NASDAQ_RNN_predictions.csv")
OUTPUT_METRICS = Path("outputs/NASDAQ_RNN_metrics.json")
OUTPUT_TRAIN_LOG = Path("outputs/NASDAQ_RNN_train_log.csv")
...
def split_train_val(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(df) < min_train_rows + 1:
        raise ValueError("NASDAQ training split is too short for RNN.")
...
class WindowDataset(Dataset):
    def __init__(self, features: np.ndarray, target: np.ndarray):
        ...
        if self.length <= 0:
            raise ValueError("Not enough rows to build RNN windows.")
...
def train_model(train_loader: DataLoader, val_loader: DataLoader, input_dim: int, device: torch.device) -> Tuple[nn.Module, List[Dict[str, float]]]:
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            "[RNN] {} | train_loss {:.6f} | val_loss {:.6f} | best {:.6f} | epoch {:.1f}s | eta {:.1f}m".format(
        ...
        if no_improve_epochs >= EARLY_STOPPING_PATIENCE:
            print(
                "[RNN] early stopping at epoch {} after {} non-improving epochs.".format(
